...
\framebox{Wording} A thread will run the CSP process of the equivalent Scala function, here referred to as |Func|. In each case these first communicates a |callFunc| before running the |syncFunc| process within a |Synchronized| block (as defined in section \ref{listing::JVMMonitorInterface}).
...
%\framebox{check that isn't an internal divergence caused by hiding spuriousWakeup}. 
